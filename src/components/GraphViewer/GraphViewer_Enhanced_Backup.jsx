...
const GraphViewerEnhanced = ({ onClose }) => {
  ...
  const generateEnhancedGraphData = useCallback(() => {
    ...
    documents.forEach((doc, index) => {
      ...
      nodes.push({
        ...
        x: x,
        y: y,
        size: 30 + (doc.blocks?.length || 0) * 2,
        color: '#3b82f6',
        data: doc
      });
      
      // 添加段落节点
      if (doc.blocks && showParagraphs) {
        doc.blocks.forEach((block, blockIndex) => {
          const blockAngle = angle + (blockIndex - doc.blocks.length / 2) * 0.3;
          const blockRadius = 80;
          const blockX = x + blockRadius * Math.cos(blockAngle);
          const blockY = y + blockRadius * Math.sin(blockAngle);
          
          nodes.push({
            id: block.id,
            type: 'block',
            label: block.content?.text?.substring(0, 30) || block.content?.label || `块 ${blockIndex + 1}`,
            category: block.type,
            x: blockX,
            y: blockY,
            size: 15,
          ...
          // 添加文档到段落的连接
          edges.push({
            id: `doc_block_${doc.id}_${block.id}`,
            from: doc.id,
            to: block.id,
            type: 'contains',
            label: '包含',
    ...
    // 添加文档关系边
    documentRelationships.forEach(rel => {
      const sourceNode = nodes.find(n => n.id === rel.sourceDocId);
      const targetNode = nodes.find(n => n.id === rel.targetDocId);
      
      if (sourceNode && targetNode) {
        edges.push({
          id: rel.id,
          from: rel.sourceDocId,
          to: rel.targetDocId,
          type: rel.type,
    ...
    blockRelationships.forEach(rel => {
      const sourceNode = nodes.find(n => n.id === rel.sourceBlockId);
      const targetNode = nodes.find(n => n.id === rel.targetBlockId);
      ...
      if (sourceNode && targetNode) {
        edges.push({
          ...
          type: rel.type,
          label: getBlockRelationshipLabel(rel.type),
          color: getBlockRelationshipColor(rel.type),
          style: 'dashed',
          weight: rel.strength || 1,
          data: rel
        });
    ...
    return {
      nodes,
      edges,
      clusters: calculateClusters(nodes, edges),
      metrics: calculateMetrics(nodes, edges)
    };
  }, [documents, documentRelationships, blockRelationships, showParagraphs]);
  ...
  // 获取段落关系类型标签
  const getBlockRelationshipLabel = (type) => {
    const labels = {
      quotes: '引用',
      elaborates: '阐述',
      contradicts: '反驳',
  ...
  // 获取段落关系类型颜色
  const getBlockRelationshipColor = (type) => {
    const colors = {
      quotes: '#10b981',
      elaborates: '#3b82f6',
      contradicts: '#ef4444',
  ...
  // 计算集群
  const calculateClusters = (nodes, edges) => {
    // 简化的集群算法：按文档分组
    const clusters = [];
    const documentNodes = nodes.filter(n => n.type === 'document');
    
    documentNodes.forEach(docNode => {
      const relatedNodes = [docNode];
      const blockNodes = nodes.filter(n => n.parentId === docNode.id);
      relatedNodes.push(...blockNodes);
      
      if (relatedNodes.length > 1) {
        clusters.push({
          id: `cluster_${docNode.id}`,
          label: docNode.label,
          nodes: relatedNodes.map(n => n.id),
          color: docNode.color,
          size: relatedNodes.length
        });
      }
    });
  ...
  const calculateMetrics = (nodes, edges) => {
    return {
      ...
      blockNodes: nodes.filter(n => n.type === 'block').length,
  ...
  const filteredData = React.useMemo(() => {
    ...
    if (searchQuery) {
      ...
      nodes = nodes.filter(node => 
        ...
        (node.data?.content?.text?.toLowerCase().includes(query))
      );
    ...
    return { nodes, edges };
  }, [enhancedGraphData, searchQuery, filterType, edgeFilter]);
        author: '张三',
        tags: ['核心', '文档', '概述'],
        content: {
          wordCount: 1200,
